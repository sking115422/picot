...
Capture a few benign agent sessions directly on the host (no Docker,
no strace, no entrypoint shell). Verify that the same attribution
prediction code recovers session/MCP/tool-call attribution from those
captures.
...
- claude CLI on PATH
- L3 sensor binary built at ds_gen/sensors/l3_libbpf/build/l3-sensor
...
Each captured session writes:
  <out_dir>/<session_id>/
    ...
    ├── l3.jsonl           L3 trace slice (ts-windowed)
...
L3_BIN = Path("/lts/ai_sec_exp/cle4as/src/sensors/l3_libbpf/build/l3-sensor")
...
def stop_l3(proc: subprocess.Popen) -> None:
    if proc.poll() is not None:
        return
    subprocess.run(["sudo", "-n", "pkill", "-TERM", "-x", "l3-sensor"],
